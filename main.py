...
textvariablelist=[]
def editUsrCfg():
    global usrEntList,pwdEntList,fromEntList,toEntList,dateEntList,nameEntList
    for i in range(7):
...
def saveCfg():
    ...
    for i in range(len(usrEntList)):
        with open('usr/usr'+str(usrCount)+'.cfg','w')as IO:
            IO.write(usrEntList[i].get()+','+pwdEntList[i].get()+','+fromEntList[i].get()+','+toEntList[i].get()+','+dateEntList[i].get()+','+nameEntList[i].get())
        usrCount+=1
def launch():
